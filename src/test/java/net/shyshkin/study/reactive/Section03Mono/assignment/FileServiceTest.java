...
class FileServiceTest {
    ...
    void read_success(String filename, String content) {
        ...
        //when
        Mono<String> mono = FileService.read(filename)
    ...
    void write_success() {
        ...
        //when
        Mono<Void> mono = FileService.write(filename, content)
                .doOnNext(System.out::println);
    ...
    void delete_success() {
        ...
        StepVerifier.create(mono)
                .verifyComplete();
        assertTrue(FileService.fileExist(filename));

        //when
        Mono<Void> deleteMono = FileService.delete(filename);

        //then
        StepVerifier.create(deleteMono)
                .verifyComplete();

        assertFalse(FileService.fileExist(filename));
    }
    ...
    void delete_fileNotFound() {
        ...
        //when
        Mono<Void> mono = FileService.delete(filename)
                .doOnError(Util.onError);
